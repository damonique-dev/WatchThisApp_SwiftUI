...
func userReducer(state: UserState, action: Action) -> UserState {
    ...
    switch(action) {
        case let action as UserActions.AddToCustomList:
            state.customLists[action.customListUUID]?.traktItems[action.slug] = TraktListItem(slug: action.slug, itemType: action.itemType)
        case let action as UserActions.RemoveFromCustomList:
            state.customLists[action.customListUUID]?.traktItems.removeValue(forKey: action.slug)
